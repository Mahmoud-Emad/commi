...
#   curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/Mahmoud-Emad/commi/main/install.sh | sh -s -- --help
#   curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/Mahmoud-Emad/commi/main/install.sh | sh -s -- --no-sudo
...
# Check if we're being run from a pipe (like curl | sh)
is_piped() {
    [ ! -t 0 ]
}

# Ensure we have secure download capabilities
ensure_secure_download() {
    if command_exists curl; then
        # Check if curl supports required security features
        if ! curl --help | grep -q "proto"; then
            log_warning "curl version may not support --proto flag"
        fi
        return 0
    elif command_exists wget; then
        # wget is acceptable but curl is preferred for security
        log_warning "wget detected. curl is recommended for better security."
        return 0
    else
        error_exit "Neither curl nor wget found. Please install curl for secure downloads."
    fi
}
...
check_prerequisites() {
    ...
    # Check if we're being run securely
    if is_piped; then
        log_info "Running from pipe (curl | sh) - ensuring secure installation"
    fi
...
get_latest_release() {
    ...
    if command_exists curl; then
        curl --proto '=https' --tlsv1.2 -sSf "$GITHUB_API" > "$temp_file" || error_exit "Failed to fetch release information"
    elif command_exists wget; then
        wget --https-only --secure-protocol=TLSv1_2 -q -O "$temp_file" "$GITHUB_API" || error_exit "Failed to fetch release information"
    fi
...
install_binary() {
    ...
    if command_exists curl; then
        curl --proto '=https' --tlsv1.2 -sSfL -o "$temp_binary" "$download_url" || error_exit "Failed to download binary"
    elif command_exists wget; then
        wget --https-only --secure-protocol=TLSv1_2 -O "$temp_binary" "$download_url" || error_exit "Failed to download binary"
    fi
...
This will download and install the latest version of commi, an AI-powered
