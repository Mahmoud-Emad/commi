...
def test_commit_changes():
    ...
    # Test commit error
    mock_repo.git.diff.return_value = "changes"
    mock_repo.git.commit.side_effect = git.exc.GitCommandError(
...
def mock_dependencies():
    ...
    with patch("git.Repo") as mock_repo, patch(
        "google.generativeai.GenerativeModel"
    ) as mock_genai, patch("google.generativeai.configure") as mock_configure, patch(
        "pyperclip.copy"
    ) as mock_copy, patch(
        "decouple.config"
    ) as mock_config:
...
def test_main_flow(mock_dependencies):
    ...
    with patch("sys.argv", ["commi", "--copy", "--api-key", "test-key"]), patch(
        "commi.cmd.CommiCommands"
    ) as mock_cmd:
